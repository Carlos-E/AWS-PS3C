					...
					function initMap() {
						...
						document
							...
							.addEventListener(
								...
								function () {
									///////ACTUALIZAR ENVIO
									let cliente = envios[document
										.getElementById('envios').selectedIndex].usuario;
									let fecha = envios[document
										.getElementById('envios').selectedIndex].fecha;
									///////ACTUALIZAR ENVIO
					...
					function calculateAndDisplayRoute(directionsService, directionsDisplay) {
						directionsService.route({
							...
							if (status === 'OK') {
								...
								if (typeof Android != 'undefined') {
									Android.showToast('Distancia: '
										+ response.routes[0].legs[0].distance.text);
									Android.showToast('Duración: '
							...
							.done(
								function (response) {
									...
									if (response !== null) {
										...
										if (envios[document
											.getElementById('envios').selectedIndex].chequeoCarga) {
											document.getElementById('recogido').checked = true;
											document.getElementById('end').value = envios[document
												.getElementById('envios').selectedIndex].destinoLatLong;
										} else {
										...
										if (envios[document
											.getElementById('envios').selectedIndex].chequeoDescarga) {
											document.getElementById('entregado').checked = true;
											directionsDisplay.setMap(null);
											if (typeof Android != 'undefined') {
												Android
													.showToast('Envio completado');
											}
										} else {
											document.getElementById('entregado').checked = false;
										}
